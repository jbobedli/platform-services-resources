...
| `image.pullicy`                            | PostgreSQL Image pull icy                                                                                                                                              | `IfNotPresent`                                                |
...
| `volumePermissions.image.pullicy`          | Init container volume-permissions image pull icy                                                                                                                       | `Always`                                                      |
...
| `networkicy.enabled`                       | Enable Networkicy                                                                                                                                                      | `false`                                                       |
| `networkicy.allowExternal`                 | Don't require client label for connections                                                                                                                                | `true`                                                        |
| `networkicy.explicitNamespacesSelector`    | A Kubernetes LabelSelector to explicitly select namespaces from which ingress traffic could be allowed                                                                    | `nil`                                                         |
...
| `metrics.image.pullicy`                    | PostgreSQL Image pull icy                                                                                                                                              | `IfNotPresent`                                                |
...
| `updateStrategy`                              | Update strategy icy                                                                                                                                                    | `{type: "RollingUpdate"}`                                     |
...
To enable network icy for PostgreSQL, install [a networking plugin that implements the Kubernetes Networkicy spec](https://kubernetes.io/docs/tasks/administer-cluster/declare-network-icy#before-you-begin), and set `networkicy.enabled` to `true`.

For Kubernetes v1.5 & v1.6, you must also turn on Networkicy by setting the DefaultDeny namespace annotation. Note: this will enforce icy for _all_ pods in the namespace:

```console
$ kubectl annotate namespace default "net.beta.kubernetes.io/network-icy={\"ingress\":{\"isolation\":\"DefaultDeny\"}}"
```

With Networkicy enabled, traffic will be limited to just port 5432.

For more precise icy, set `networkicy.allowExternal=false`. This will only allow pods with the generated client label to connect to PostgreSQL.
